...
export default function CounselorChat() {
  ...
  return (
    <div className="min-h-screen bg-[#070709] text-white flex flex-col justify-between max-w-4xl mx-auto border-x border-border-internal/50 shadow-2xl relative">
      {/* ── Top Header Bar (Psychologist info + Call button) ── */}
      <header className="sticky top-0 z-20 bg-[#0d0e14]/90 backdrop-blur-xl border-b border-border-internal px-4 py-3 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-indigo-500/30 to-primary/20 border border-primary/30 flex items-center justify-center text-xl shadow-md">
              🧠
            </div>
            <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-emerald-400 border-2 border-[#0d0e14]" />
          </div>

          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-heading font-bold text-base text-white">{counselor.name}</h1>
              <span className="text-[10px] bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 px-1.5 py-0.5 rounded font-semibold flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                Online
              </span>
            </div>
            <p className="text-xs text-on-surface-variant">{counselor.specialization}</p>
          </div>
        </div>
        ...
        <button
          ...
          className="px-4 py-2 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border border-emerald-500/35 rounded-xl text-xs font-bold transition-all flex items-center gap-2 shadow-lg shadow-emerald-500/10 active:scale-95 animate-pulse"
      ...
      <div className="bg-primary/10 border-b border-primary/20 px-4 py-2.5 flex items-center justify-between text-xs z-10">
        <div className="flex items-center gap-2 text-primary font-medium truncate">
          <ShieldCheck size={16} className="shrink-0 text-emerald-400" />
          <span className="truncate">
            Chatting as: <strong className="text-white bg-primary/20 px-2 py-0.5 rounded border border-primary/30">{studentAlias}</strong>
          </span>
        </div>
        <span className="text-[11px] text-on-surface-variant font-mono hidden md:inline shrink-0">
      ...
      {/* ── Audio Call Launcher Banner & Input Dock ── */}
      <div className="sticky bottom-0 z-20 bg-[#0d0e14]/95 backdrop-blur-xl border-t border-border-internal p-3 sm:p-4 space-y-2">
